		...
		for _, gpu_id in ipairs(requested_gpus) do
			...
			if gpu_data then
				...
				if state.user then
					...
					elseif state.type == "run" and state.last_heartbeat and (current_time - tonumber(state.last_heartbeat)) > 120 then
						-- Run reservation heartbeat timeout (2 minutes), continue
					else
